...
<body>
    ...
    //get all user
    $sqlStr = "SELECT noteperm.UserID_FK, user.Username FROM noteperm INNER JOIN user ON noteperm.UserID_FK = user.UserID AND noteperm.NoteID_FK = '" . $_SESSION['NoteID'] . "' ORDER BY UserID_FK ASC";
    $sqlQuery = mysqli_query($dbConnect, $sqlStr);
    ?>
    <a href="editNote.php">Back</a>
    <br>
    <table border="1">
        <tr>
            <th>UserID</th>
        ...
        while ($sqlResult = mysqli_fetch_array($sqlQuery)) { ?>
            <tr>
                ...
                <td><a href="delNotePerm.php?UserID=<?php echo $sqlResult["UserID_FK"] ?>" onclick="return confirm('Are you sure?')">DEL</a></td>
            </tr>
        <?php } ?>
    </table>
    <form name="addUser" method="post" action="addNotePerm.php">
